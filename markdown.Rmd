...
It was decided that we would keep bird data that were observed more than 4 times, and that match between the two periods (roaming and nesting). 
```{r}
#match
...
We must now extract the data for parks and greens areas (which represents in the data set the code 600 in UTIL_SOL) and golfs (1100) and keep only areas greater than 0,5 ha which is 5 000 m2. 
...
```{r}
bird_nest_green <- st_join(comm_nest, green_areas, join = st_within) 
mapview(bird_nest_green)+green_areas

```
It doesn't work since some bird count points are found out of the polygons. 
